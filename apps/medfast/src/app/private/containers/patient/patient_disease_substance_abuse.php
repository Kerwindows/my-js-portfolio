...
$patientID = $_POST['patientID'];  // Default ID or fetched dynamically from javascript
$conditionsArray = getConditionForm('substance_abuse'); 
$conditionValues = getConditionValues($patientID,'substance_abuse_log'); 
...
<div class="form-heading">
    <h4>Substance Abuse Information</h4>
</div>
<form id="substanceAbuseForm" method="post"  >
    <input type="hidden" name="patientID" value="<?= htmlspecialchars($patientID); ?>">
    <input type="hidden" name="tbl" value="<?= htmlspecialchars('substance_abuse_log'); ?>">
    <div class="row">
        <?php foreach ($conditionsArray as $disease): ?>
        <div class="<?= $disease['level'] > 1 ? 'col-md-4' : 'col-12'; ?> mb-4">
